...
test(
  'prepared PDF is uploaded as multipart through the same job HTTP client',
  async () => {
    ...
    assert.equal(
      receivedOptions.maxRedirections,
      0,
    );


    assert.equal(
      receivedOptions.headers['Content-Type']
        .startsWith(
...
test(
  'workflow cannot manually override multipart Content-Type',
  async () => {
...
test(
  'challenge during PDF upload requires manual handling',
  async () => {
